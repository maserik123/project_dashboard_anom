<script>
    ...
    $(document).ready(function() {
        table = $('#projectMaster').DataTable({
            ...
            "dataType": 'JSON',
            "ajax": {
                "url": "<?php echo site_url('administrator/projectPICHeader/getAllData') ?>",
                "type": "POST",
                "data": {
    ...
    function updateData(id) {
        ...
        //Load data dari ajax
        $.ajax({
            url: "<?php echo base_url('administrator/projectPICHeader/getById/'); ?>" + id,
            type: "GET",
            dataType: "JSON",
            success: function(resp) {
                ...
                $('#modalMasterProject').modal('show');
                $('.modal-title').text('Edit Data PIC Project Hdr');
    ...
    function deleteData(id) {
        swal({
            ...
            if (result == true) {
                $.ajax({
                    url: "<?php echo site_url('administrator/projectPICHeader/delete'); ?>/" + id,
                    type: "POST",
                    data: {
                        '<?php echo $this->security->get_csrf_token_name(); ?>': '<?php echo $this->security->get_csrf_hash(); ?>'
                    },
    ...
    function save() {
        ...
        if (save_method_role == 'add') {
            url = '<?php echo base_url() ?>administrator/projectPICHeader/insert';
        } else {
            url = '<?php echo base_url() ?>administrator/projectPICHeader/update';
        }
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title">PIC Project Header</h3>
        ...
        <div class="modal-content">
            ...
            <form class="form-horizontal" id="formMasterProject" action="" method="POST">
                <div class="modal-body">
                    <input type="hidden" name='pic_project_hdr_id' value="" id='pic_project_hdr_id'>
                    <div class="item form-group">
                        <label class="control-label col-md-12 col-sm-3 col-xs-12">Criteria Project Name <span class="required">*</span>
                        </label>
                        <div class="col-md-12 col-sm-9 col-xs-12">
                            <input type="text" id="pic_project_name" name="pic_project_name" placeholder="PIC Project Name" required="required" class="form-control ">
                        </div>
                    </div>
                </div>
